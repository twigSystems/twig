...
async def processar_periodo(call_or_message, loja, periodo, inicio=None, fim=None, todas_lojas=False, grupo=None):
    ...
    try:
        ...
        logger.info(f"Período selecionado: {periodo} para {'grupo' if todas_lojas else 'loja'} {loja} por {user_info}")

        try:
            await bot.edit_message_reply_markup(chat_id, message_id, reply_markup=None)
        except MessageNotModified:
            logger.warning(f"Mensagem não modificada: {message_id}")
        except MessageCantBeEdited:
            logger.warning(f"Mensagem não pode ser editada: {message_id}")

        mensagem_carregando = await bot.send_message(chat_id, "⏳ Carregando os dados, por favor aguarde um momento.")

        if todas_lojas:
            resultados = consultar_dados_acumulados_por_grupo(grupo, inicio, fim)
        else:
        ...
        resposta = (
            f"{escape_md(saudacao)}, {escape_md(user_info.split(':')[1].split(',')[0])}! 🌞\n\n"
            f"*Resumo para {'grupo' if todas_lojas else 'loja'} {escape_md(loja)} de {inicio.strftime('%Y-%m-%d %H:%M')} a {fim.strftime('%Y-%m-%d %H:%M')}:* 📊\n\n"
            f"*Indicadores de Desempenho:* 📈\n\n"
        )

        if todas_lojas:
            resposta += (
                f"{escape_md(mostrar_resultados(resultados['total_vendas_com_iva'], 0, 'Total de Vendas (c/ IVA)', monetario=True))}\n"
                f"{escape_md(mostrar_resultados(resultados['total_vendas_sem_iva'], 0, 'Total de Vendas (s/ IVA)', monetario=True))}\n"
                f"{escape_md(mostrar_resultados(resultados['transacoes_vendas'], 0, 'Transações'))}\n"
                f"{escape_md(mostrar_resultados(resultados['visitantes'], 0, 'Visitantes'))}\n"
                f"{escape_md(mostrar_resultados_percentual(resultados['taxa_conversao'], 0, 'Taxa de Conversão'))}\n"
                f"{escape_md(mostrar_resultados_minutos(resultados['tempo_medio_permanencia'], 0, 'Tempo Médio de Permanência'))}\n"
                f"{escape_md(mostrar_resultados(resultados['total_passagens'], 0, 'Número de Passagens'))}\n"
                f"{escape_md(mostrar_resultados_percentual(resultados['entry_rate'], 0, 'Taxa de Captação'))}\n\n"
                f"*Indicadores de Eficiência:* 🛠️\n\n"
                f"{escape_md(mostrar_resultados(resultados['ticket_medio_com_iva'], 0, 'Ticket Médio (c/ IVA)', monetario=True))}\n"
                f"{escape_md(mostrar_resultados(resultados['ticket_medio_sem_iva'], 0, 'Ticket Médio (s/ IVA)', monetario=True))}\n"
                f"{escape_md(mostrar_resultados_unidades(resultados['unidades_por_transacao'], 0, 'Unidades por Transação'))}\n"
                f"{escape_md(mostrar_resultados_devolucoes(resultados['indice_devolucoes'], 0, 'Índice de Devoluções'))}\n"
                f"{escape_md(mostrar_resultados_descontos(resultados['indice_descontos'], 0, 'Índice de Descontos'))}\n\n"
                f"*Top Vendedores (s/IVA):* 🏅\n\n"
            )

            for vendedor, valor in resultados['top_vendedores']:
                resposta += f"{escape_md(vendedor)}: €{valor:.0f}\n"

            resposta += "\n*Top Produtos (Qtd):* 🛒\n\n"
            for produto, quantidade in resultados['top_produtos']:
                resposta += f"{escape_md(produto)}: {quantidade:.0f} u.\n"

            resposta += f"\nÚltima atualização dos dados: {escape_md(resultados['ultima_coleta'].strftime('%Y-%m-%d %H:%M'))} 📅"
        else:
            resposta += (
                f"{escape_md(mostrar_resultados(resultados['resultados_atuais']['total_vendas_com_iva'], resultados['resultados_anteriores']['total_vendas_com_iva'], 'Total de Vendas (c/ IVA)', monetario=True))}\n"
                f"{escape_md(mostrar_resultados(resultados['resultados_atuais']['total_vendas_sem_iva'], resultados['resultados_anteriores']['total_vendas_sem_iva'], 'Total de Vendas (s/ IVA)', monetario=True))}\n"
                f"{escape_md(mostrar_resultados(resultados['resultados_atuais']['transacoes_vendas'], resultados['resultados_anteriores']['transacoes_vendas'], 'Transações'))}\n"
                f"{escape_md(mostrar_resultados(resultados['resultados_atuais']['visitantes'], resultados['resultados_anteriores']['visitantes'], 'Visitantes'))}\n"
                f"{escape_md(mostrar_resultados_percentual(resultados['resultados_atuais']['taxa_conversao'], resultados['resultados_anteriores']['taxa_conversao'], 'Taxa de Conversão'))}\n"
                f"{escape_md(mostrar_resultados_minutos(resultados['resultados_atuais']['tempo_medio_permanencia'], resultados['resultados_anteriores']['tempo_medio_permanencia'], 'Tempo Médio de Permanência'))}\n"
                f"{escape_md(mostrar_resultados(resultados['resultados_atuais']['total_passagens'], resultados['resultados_anteriores']['total_passagens'], 'Número de Passagens'))}\n"
                f"{escape_md(mostrar_resultados_percentual(resultados['resultados_atuais']['entry_rate'], resultados['resultados_anteriores']['entry_rate'], 'Taxa de Captação'))}\n\n"
                f"*Indicadores de Eficiência:* 🛠️\n\n"
                f"{escape_md(mostrar_resultados(resultados['resultados_atuais']['ticket_medio_com_iva'], resultados['resultados_anteriores']['ticket_medio_com_iva'], 'Ticket Médio (c/ IVA)', monetario=True))}\n"
                f"{escape_md(mostrar_resultados(resultados['resultados_atuais']['ticket_medio_sem_iva'], resultados['resultados_anteriores']['ticket_medio_sem_iva'], 'Ticket Médio (s/ IVA)', monetario=True))}\n"
                f"{escape_md(mostrar_resultados_unidades(resultados['resultados_atuais']['unidades_por_transacao'], resultados['resultados_anteriores']['unidades_por_transacao'], 'Unidades por Transação'))}\n"
                f"{escape_md(mostrar_resultados_devolucoes(resultados['resultados_atuais']['indice_devolucoes'], resultados['resultados_anteriores']['indice_devolucoes'], 'Índice de Devoluções'))}\n"
                f"{escape_md(mostrar_resultados_descontos(resultados['resultados_atuais']['indice_descontos'], resultados['resultados_anteriores']['indice_descontos'], 'Índice de Descontos'))}\n\n"
                f"*Top Vendedores (s/IVA):* 🏅\n\n"
            )
            
            for vendedor, valor in resultados['resultados_atuais']['top_vendedores']:
                resposta += f"{escape_md(vendedor)}: €{valor:.0f}\n"

            resposta += "\n*Top Produtos (Qtd):* 🛒\n\n"
            for produto, quantidade in resultados['resultados_atuais']['top_produtos']:
                resposta += f"{escape_md(produto)}: {quantidade:.0f} u.\n"

            resposta += f"\nÚltima atualização dos dados: {escape_md(resultados['resultados_atuais']['ultima_coleta'].strftime('%Y-%m-%d %H:%M'))} 📅"
        ...
        if mensagem_carregando:
            try:
                ...
                logger.error("Mensagem a deletar não encontrada.")
...
async def main():
    ...
    last_chat_id = get_last_chat_id()
    if last_chat_id:
        await enviar_mensagem_reinicio(last_chat_id)
