...
async def process_consultar_grupo(call: types.CallbackQuery):
    try:
        ...
        for loja in lojas:
            markup.add(InlineKeyboardButton(loja, callback_data=f"consultar_loja:{loja}"))
        markup.add(InlineKeyboardButton("Todas as Lojas", callback_data=f"consultar_grupo_todas:{grupo}"))
        markup.add(InlineKeyboardButton("Saír", callback_data="sair"))
        await bot.send_message(call.message.chat.id, "🏬 Selecione a loja que deseja consultar:", reply_markup=markup)
...
async def process_periodo_step(call: types.CallbackQuery):
    ...
    if periodo == "Customizado":
        ...
        if loja == "Todas as Lojas":
            await processar_periodo(call.message, loja, periodo, todas_lojas=True, grupo=grupo)
        else:
...
async def process_periodo_step(call: types.CallbackQuery):
    ...
    if periodo == "Customizado":
        ...
        if loja == "Todas as Lojas":
            await processar_periodo(call.message, grupo, periodo, todas_lojas=True)
        else:
            await processar_periodo(call.message, loja, periodo)
...
async def processar_periodo(call_or_message, loja, periodo, inicio=None, fim=None, todas_lojas=False, grupo=None):
    ...
    try:
        ...
        resposta = (
            f"{saudacao}, {user_info.split(':')[1].split(',')[0]}! 🌞\n\n"
            f"**Resumo para {'grupo' if todas_lojas else 'loja'} {loja} de {inicio.strftime('%Y-%m-%d %H:%M')} a {fim.strftime('%Y-%m-%d %H:%M')}:** 📊\n\n"
            f"**Indicadores de Desempenho:** 📈\n\n"
        )

        if todas_lojas:
            resposta += (
                f"{mostrar_resultados(resultados['total_vendas_com_iva'], 0, 'Total de Vendas (c/ IVA)', monetario=True)}\n"
                f"{mostrar_resultados(resultados['total_vendas_sem_iva'], 0, 'Total de Vendas (s/ IVA)', monetario=True)}\n"
                f"{mostrar_resultados(resultados['transacoes_vendas'], 0, 'Transações')}\n"
                f"{mostrar_resultados(resultados['visitantes'], 0, 'Visitantes')}\n"
                f"{mostrar_resultados_percentual(resultados['taxa_conversao'], 0, 'Taxa de Conversão')}\n"
                f"{mostrar_resultados_minutos(resultados['tempo_medio_permanencia'], 0, 'Tempo Médio de Permanência')}\n"
                f"{mostrar_resultados(resultados['total_passagens'], 0, 'Número de Passagens')}\n"
                f"{mostrar_resultados_percentual(resultados['entry_rate'], 0, 'Taxa de Captação')}\n\n"
                f"**Indicadores de Eficiência:** 🛠️\n\n"
                f"{mostrar_resultados(resultados['ticket_medio_com_iva'], 0, 'Ticket Médio (c/ IVA)', monetario=True)}\n"
                f"{mostrar_resultados(resultados['ticket_medio_sem_iva'], 0, 'Ticket Médio (s/ IVA)', monetario=True)}\n"
                f"{mostrar_resultados_unidades(resultados['unidades_por_transacao'], 0, 'Unidades por Transação')}\n"
                f"{mostrar_resultados_devolucoes(resultados['indice_devolucoes'], 0, 'Índice de Devoluções')}\n"
                f"{mostrar_resultados_descontos(resultados['indice_descontos'], 0, 'Índice de Descontos')}\n\n"
            ...
            for produto, quantidade in resultados['top_produtos']:
                resposta += f"{produto}: {quantidade:.0f} u.\n"

            resposta += f"\nÚltima atualização dos dados: {resultados['ultima_coleta'].strftime('%Y-%m-%d %H:%M')} 📅"
        else:
            resposta += (
                f"{mostrar_resultados(resultados['resultados_atuais']['total_vendas_com_iva'], resultados['resultados_anteriores']['total_vendas_com_iva'], 'Total de Vendas (c/ IVA)', monetario=True)}\n"
                f"{mostrar_resultados(resultados['resultados_atuais']['total_vendas_sem_iva'], resultados['resultados_anteriores']['total_vendas_sem_iva'], 'Total de Vendas (s/ IVA)', monetario=True)}\n"
                f"{mostrar_resultados(resultados['resultados_atuais']['transacoes_vendas'], resultados['resultados_anteriores']['transacoes_vendas'], 'Transações')}\n"
                f"{mostrar_resultados(resultados['resultados_atuais']['visitantes'], resultados['resultados_anteriores']['visitantes'], 'Visitantes')}\n"
                f"{mostrar_resultados_percentual(resultados['resultados_atuais']['taxa_conversao'], resultados['resultados_anteriores']['taxa_conversao'], 'Taxa de Conversão')}\n"
                f"{mostrar_resultados_minutos(resultados['resultados_atuais']['tempo_medio_permanencia'], resultados['resultados_anteriores']['tempo_medio_permanencia'], 'Tempo Médio de Permanência')}\n"
                f"{mostrar_resultados(resultados['resultados_atuais']['total_passagens'], resultados['resultados_anteriores']['total_passagens'], 'Número de Passagens')}\n"
                f"{mostrar_resultados_percentual(resultados['resultados_atuais']['entry_rate'], resultados['resultados_anteriores']['entry_rate'], 'Taxa de Captação')}\n\n"
                f"**Indicadores de Eficiência:** 🛠️\n\n"
                f"{mostrar_resultados(resultados['resultados_atuais']['ticket_medio_com_iva'], resultados['resultados_anteriores']['ticket_medio_com_iva'], 'Ticket Médio (c/ IVA)', monetario=True)}\n"
                f"{mostrar_resultados(resultados['resultados_atuais']['ticket_medio_sem_iva'], resultados['resultados_anteriores']['ticket_medio_sem_iva'], 'Ticket Médio (s/ IVA)', monetario=True)}\n"
                f"{mostrar_resultados_unidades(resultados['resultados_atuais']['unidades_por_transacao'], resultados['resultados_anteriores']['unidades_por_transacao'], 'Unidades por Transação')}\n"
                f"{mostrar_resultados_devolucoes(resultados['resultados_atuais']['indice_devolucoes'], resultados['resultados_anteriores']['indice_devolucoes'], 'Índice de Devoluções')}\n"
                f"{mostrar_resultados_descontos(resultados['resultados_atuais']['indice_descontos'], resultados['resultados_anteriores']['indice_descontos'], 'Índice de Descontos')}\n\n"
                f"**Top Vendedores (s/IVA):** 🏅\n\n"
            )
            ...
            resposta += f"\nÚltima atualização dos dados: {resultados['resultados_atuais']['ultima_coleta'].strftime('%Y-%m-%d %H:%M')} 📅"
